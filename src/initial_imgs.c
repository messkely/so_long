...
/*   Updated: 2024/05/21 09:03:00 by messkely         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/so_long.h"

static	void	init_left_txts(t_mlx *my_mlx)
{
	if (!my_mlx->coin_txt)
	{
	...
	my_mlx->exit_txt = mlx_load_png("textures/exit.png");
	if (!my_mlx->exit_txt)
	{
		mlx_delete_texture(my_mlx->bg_txt);
		mlx_delete_texture(my_mlx->wall_txt);
...
static	void	init_txts(t_mlx *my_mlx)
{
	my_mlx->bg_txt = mlx_load_png("textures/background.png");
	if (!my_mlx->bg_txt)
	{
		mlx_terminate(my_mlx->mlx_ptr);
		exit(1);
	}
	my_mlx->wall_txt = mlx_load_png("textures/wall.png");
	if (!my_mlx->wall_txt)
	{
		mlx_delete_texture(my_mlx->bg_txt);
		mlx_terminate(my_mlx->mlx_ptr);
	...
	my_mlx->player_txt = mlx_load_png("textures/player.png");
	...
	my_mlx->coin_txt = mlx_load_png("textures/coin.png");
...
static	void	resize_imgs(t_mlx *mlx)
{
...
void	init_imgs(t_mlx *my_mlx)
{
	...
	my_mlx->player_img
		= mlx_texture_to_image(my_mlx->mlx_ptr, my_mlx->player_txt);
